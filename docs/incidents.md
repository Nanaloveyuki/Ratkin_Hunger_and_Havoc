...
显示 ID 是目录数据，不是列表下标。`I-001`..`I-051` 都有 IncidentDef 和 Worker。`baseChance` 为 0，不进入原版类别池。`RHAH_IncidentSchedule` 在讲述者每次检查时按正负两个平均天数抽池，再用 `RHAH_IncidentWeight` 选一条，交给现有 `QueueIncident`。开发者事件页也只入这条队列，不在菜单里 `TryExecute`，不掷权重。IrisMenus 强制暂停，游戏 tick 停着。排队后由 `GameComponentUpdate` 在暂停窗口都关掉的下一帧生成，不等时间继续走。地图事件打当前可用地图，`I-035` 与 `I-050` 打玩家商队。开发者触发会写完整日志：拒绝原因、入队的 def 与点数、生成成功、`TryExecute` 失败或抛错。失败时事件留在队列。
